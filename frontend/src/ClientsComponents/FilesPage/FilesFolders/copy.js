...
import { addFilesAndFolders } from "../../../features/actions/filesAndFolders";

const folders = [
  { id: 1, name: "Health Insurance", type: "folder", isDeleteAllowed: false },
  { id: 2, name: "Home Loan", type: "folder", isDeleteAllowed: false },
  { id: 3, name: "Whole Life Insurance", type: "folder", isDeleteAllowed: false },
  { id: 4, name: "Vehicle Loan", type: "folder", isDeleteAllowed: false },
  { id: 5, name: "Home Loan", type: "folder", isDeleteAllowed: false },
];
...
const FilesFolders = () => {
  ...
  function renameFolder(index, newLabel) {
    ...
    setFolderData((prevFolders) => {
      ...
      updatedFolders[index].name = newLabel;

    dispatch(addFilesAndFolders({userId:loggedInUserData?._id,folderName:newLabel}))
  ...
  const createFolder = () => {
    setFolderData((prev) => [
      ...prev,
      { id: 6, name: "New Folder", type: "folder", isDeleteAllowed: true },
    ]);
  };
...
const FolderIcon = (props) => {
  ...
  const [newLabel, setNewLabel] = useState(folder.name);

  function handleClick() {
    if (!isEditing) {
      setRoutes(`${prefix}/${folder.name}`);
    }
  }
